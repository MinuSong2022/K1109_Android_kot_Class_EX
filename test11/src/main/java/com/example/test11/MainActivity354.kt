...
class MainActivity354 : AppCompatActivity() {
    ...
    class MyPagerAdapter(val datas: MutableList<String>): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            // Item354Binding.xml 파일 안에 아이디가정의 -> item_pager_textView
            // 다시, 바인딩을 하고  정의후 선택해서 사용함.
            // -> itemPagerTextView
            val binding=(holder as MyPagerViewHolder).binding
            //뷰에 데이터 출력
            binding.itemPagerTextView.text=datas[position]
            when(position % 3){
                0 -> binding.itemPagerTextView.setBackgroundColor(Color.CYAN)
                1 -> binding.itemPagerTextView.setBackgroundColor(Color.TRANSPARENT)
