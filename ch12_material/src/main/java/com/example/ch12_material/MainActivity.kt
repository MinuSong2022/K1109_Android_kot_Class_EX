...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.mainDrawerView.setNavigationItemSelectedListener {
            ...
            Log.d("song", "제목 찍어보기 : ${it.title}")
                when(it.title){
                    "Login"->
                 /*       Toast.makeText(this@MainActivity, "로그인 페이지로이동", Toast.LENGTH_SHORT).show()*/
                    startActivity(intent)

                    "회원가입" ->
                        Toast.makeText(this@MainActivity, "회원가입 페이지로이동", Toast.LENGTH_SHORT).show()
                }
                    true
